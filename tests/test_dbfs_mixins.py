def test_moving_dbfs_file_to_local_dir(config, tmp_path, mocker):
    ...
    from databricks.sdk.service.dbfs import FileInfo, ReadResponse

    get_status = mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.get_status',
    ...
    mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.read', wraps=fake_read)
    delete = mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.delete')
...
def test_moving_local_dir_to_dbfs(config, tmp_path, mocker):
    ...
    from databricks.sdk.service.dbfs import CreateResponse

    with (tmp_path / 'a').open('wb') as f:
        f.write(b'hello')

    mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.create', return_value=CreateResponse(123))

    def fake(path: str):
        assert path == 'a'
        raise DatabricksError('nope', error_code='RESOURCE_DOES_NOT_EXIST')

    mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.get_status', wraps=fake)
    add_block = mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.add_block')
    close = mocker.patch('databricks.sdk.service.dbfs.DbfsAPI.close')
