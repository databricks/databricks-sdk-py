...
def test_open_ai_client(monkeypatch):  # type: ignore[no-untyped-def]
    from databricks.sdk import WorkspaceClient

    monkeypatch.setenv("DATABRICKS_HOST", "test_host")
    monkeypatch.setenv("DATABRICKS_TOKEN", "test_token")
    w = WorkspaceClient(config=Config())
    client = w.serving_endpoints.get_open_ai_client()  # type: ignore[no-untyped-call]

    assert client.base_url == "https://test_host/serving-endpoints/"
    assert client.api_key == "no-token"


def test_open_ai_client_with_custom_params(monkeypatch):  # type: ignore[no-untyped-def]
    from databricks.sdk import WorkspaceClient
    ...
    client = w.serving_endpoints.get_open_ai_client(timeout=30.0, max_retries=3)  # type: ignore[no-untyped-call]
...
def test_open_ai_client_with_additional_kwargs(monkeypatch):  # type: ignore[no-untyped-def]
    from databricks.sdk import WorkspaceClient
...
def test_open_ai_client_prevents_reserved_param_override(monkeypatch):  # type: ignore[no-untyped-def]
    from databricks.sdk import WorkspaceClient
    ...
    with pytest.raises(ValueError, match="Cannot override reserved Databricks parameters: base_url"):
        w.serving_endpoints.get_open_ai_client(base_url="https://custom-host")  # type: ignore[no-untyped-call]
    ...
    with pytest.raises(ValueError, match="Cannot override reserved Databricks parameters: api_key"):
        w.serving_endpoints.get_open_ai_client(api_key="custom-key")  # type: ignore[no-untyped-call]
    ...
    with pytest.raises(ValueError, match="Cannot override reserved Databricks parameters: http_client"):
        w.serving_endpoints.get_open_ai_client(http_client=None)  # type: ignore[no-untyped-call]
    ...
    with pytest.raises(ValueError, match="Cannot override reserved Databricks parameters: api_key, base_url"):
        w.serving_endpoints.get_open_ai_client(base_url="https://custom-host", api_key="custom-key")  # type: ignore[no-untyped-call]


@pytest.mark.skipif(sys.version_info < (3, 8), reason="Requires Python > 3.7")
def test_langchain_open_ai_client(monkeypatch):  # type: ignore[no-untyped-def]
    from databricks.sdk import WorkspaceClient

    monkeypatch.setenv("DATABRICKS_HOST", "test_host")
    monkeypatch.setenv("DATABRICKS_TOKEN", "test_token")
    w = WorkspaceClient(config=Config())
    client = w.serving_endpoints.get_langchain_chat_open_ai_client("databricks-meta-llama-3-1-70b-instruct")  # type: ignore[no-untyped-call]
...
def test_http_request(w, requests_mock):  # type: ignore[no-untyped-def]
    headers = {
        "Accept": "text/plain",
