def test_get_workspace_client(ucacct, env_or_skip):  # type: ignore[no-untyped-def]
...
def test_get_workspace_id(ucws, env_or_skip):  # type: ignore[no-untyped-def]
    ws_id = int(env_or_skip("THIS_WORKSPACE_ID"))
    assert ucws.get_workspace_id() == ws_id


def test_creating_ws_client_from_ac_client_does_not_override_config(ucacct, env_or_skip):  # type: ignore[no-untyped-def]
