...
class MultipartUploadTestCase:
    ...
    expired_url_aws_response = ('<?xml version="1.0" encoding="utf-8"?><Error><Code>'
    ...
    expired_url_azure_response = ('<?xml version="1.0" encoding="UTF-8"?>\n<Error><Code>AccessDenied'
    ...
    def setup_session_mock(self, session_mock: requests_mock.Mocker,
        ...
        def custom_matcher(request):
            ...
            # initial request
            if (request_url.hostname == 'localhost'
                    and request_url.path == f'/api/2.0/fs/files{MultipartUploadTestCase.path}'
                    and request_query.get('action') == ['initiate-upload'] and request.method == 'POST'):
            ...
            # multipart upload, create upload part URLs
            elif (request_url.hostname == 'localhost'
                  and request_url.path == '/api/2.0/fs/create-upload-part-urls' and request.method == 'POST'):

                assert MultipartUploadTestCase.is_auth_header_present(request)

                request_json = request.json()
                assert request_json.keys() == {
                    'count', 'expire_time', 'path', 'session_token', 'start_part_number'
                }
                ...
                expire_time = MultipartUploadTestCase.parse_and_validate_expire_time(
                    request_json['expire_time'])

                def processor():
                    response_nodes = []
                    for part_number in range(start_part_number, start_part_number + count):
                        upload_part_url = server_state.create_upload_chunk_url(
                            self.path, part_number, expire_time)
            ...
            # multipart upload, uploading part
                    ...
                    MultipartUploadServerState.upload_chunk_url_prefix) and request.method == 'PUT':
            ...
            # multipart upload, completion
                  ...
                  and request_query.get('upload_type') == ['multipart'] and request.method == 'POST'):
            ...
            # abort upload
                    ...
                    MultipartUploadServerState.abort_upload_url_prefix) and request.method == 'DELETE':
                assert not MultipartUploadTestCase.is_auth_header_present(request)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        MultipartUploadTestCase('Initiate: intermittent retryable exception',
                                stream_size=1024 * 1024,
                                custom_response_on_initiate=CustomResponse(exception=requests.ConnectionError,
                                                                           first_invocation=1,
                                                                           last_invocation=3),
                                expected_aborted=False),
        MultipartUploadTestCase('Initiate: intermittent retryable error',
                                stream_size=1024 * 1024,
                                custom_response_on_initiate=CustomResponse(
                                    code=429, first_invocation=1, last_invocation=3),
                                expected_aborted=False),

        # -------------------------- failures on "create upload URL" --------------------------
        MultipartUploadTestCase('Create upload URL: client error is not retied',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(code=400,
                                                                                       only_invocation=1),
                                expected_exception_type=BadRequest,
                                expected_aborted=True),
        MultipartUploadTestCase('Create upload URL: internal error is not retied',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(code=500,
                                                                                       only_invocation=1),
                                expected_exception_type=InternalError,
                                expected_aborted=True),
        MultipartUploadTestCase('Create upload URL: non-JSON response is not retried',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(
                                    body='this is not a JSON', only_invocation=1),
                                expected_exception_type=requests.exceptions.JSONDecodeError,
                                expected_aborted=True),
        MultipartUploadTestCase('Create upload URL: meaningless JSON response is not retried',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(body='{"foo":123}',
                                                                                       only_invocation=1),
                                expected_exception_type=ValueError,
                                expected_aborted=True),
        MultipartUploadTestCase('Create upload URL: meaningless JSON response is not retried 2',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(
                                    body='{"upload_part_urls":[]}', only_invocation=1),
                                expected_exception_type=ValueError,
                                expected_aborted=True),
        MultipartUploadTestCase(
            'Create upload URL: meaningless JSON response is not retried 3',
            stream_size=1024 * 1024,
            custom_response_on_create_multipart_url=CustomResponse(body='{"upload_part_urls":[{"url":""}]}',
                                                                   only_invocation=1),
        ...
        MultipartUploadTestCase(
            ...
            expected_exception_type=TimeoutError,
            expected_aborted=True),
        MultipartUploadTestCase('Create upload URL: intermittent retryable exception',
                                stream_size=1024 * 1024,
                                custom_response_on_create_multipart_url=CustomResponse(
                                    exception=requests.Timeout, only_invocation=1),
                                expected_aborted=False),
        MultipartUploadTestCase(
        ...
        MultipartUploadTestCase(
            ...
            custom_response_on_abort=CustomResponse(exception=requests.Timeout,
...
class SingleShotUploadTestCase:
    ...
    def __init__(self, name: str, stream_size: int, multipart_upload_min_stream_size: int,
                 expected_single_shot: bool):
        self.name = name
        self.stream_size = stream_size
    ...
    def run(self, config: Config):
        ...
        with requests_mock.Mocker(session=session) as session_mock:
            session_mock.get(f'http://localhost/api/2.0/fs/files{MultipartUploadTestCase.path}',
                             status_code=200)
...
class ResumableUploadTestCase:
    ...
    def setup_session_mock(self, session_mock: requests_mock.Mocker,
                           server_state: ResumableUploadServerState):
        ...
        def custom_matcher(request):
            ...
            # resumable upload, uploading part
                    ...
                    ResumableUploadServerState.resumable_upload_url_prefix) and request.method == 'PUT':
            ...
            # abort upload
                    ...
                    ResumableUploadServerState.resumable_upload_url_prefix) and request.method == 'DELETE':
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ResumableUploadTestCase(
            ...
            stream_size=1024 * 1024,
            custom_response_on_create_resumable_url=CustomResponse(body='{"upload_part_urls":[{"url":""}]}',
                                                                   only_invocation=1),
            expected_exception_type=ValueError,
            expected_aborted=False # upload didn't start
        ),
        ...
        ResumableUploadTestCase(
            ...
            stream_size=1024 * 1024,
            custom_response_on_upload=CustomResponse(exception=requests.ConnectionError,
                                                     exception_happened_before_processing=False),
        ...
        # -------------- file already exists --------------
        ResumableUploadTestCase('File already exists',
                                stream_size=1024 * 1024,
                                overwrite=False,
        ...
        # -------------- success cases --------------
        ResumableUploadTestCase('Multiple chunks, zero unconfirmed delta',
                                stream_size=100 * 1024 * 1024,
                                multipart_upload_chunk_size=7 * 1024 * 1024 + 566,
                                unconfirmed_delta=0,
                                expected_aborted=False),
        ResumableUploadTestCase('Multiple chunks, non-zero unconfirmed delta',
                                stream_size=100 * 1024 * 1024,
                                multipart_upload_chunk_size=7 * 1024 * 1024 + 566,
                                unconfirmed_delta=239,
                                expected_aborted=False),
        ResumableUploadTestCase('Multiple chunks, variable unconfirmed delta',
                                stream_size=100 * 1024 * 1024,
                                multipart_upload_chunk_size=7 * 1024 * 1024 + 566,
