...
from .iam import (AccessControlAPI, AccountAccessControlAPI,
                  AccountAccessControlProxyAPI, AccountGroupsAPI,
                  AccountServicePrincipalsAPI, AccountUsersAPI, CurrentUserAPI,
                  GroupsAPI, PermissionMigrationAPI, PermissionsAPI,
                  ServicePrincipalsAPI, UsersAPI, WorkspaceAssignmentAPI)

_LOG = logging.getLogger(__name__)


class AccessControlClient(AccessControlAPI):
    """
...
class AccountAccessControlClient(AccountAccessControlAPI):
    """
...
class AccountAccessControlProxyClient(AccountAccessControlProxyAPI):
    """
    These APIs manage access rules on resources in an account. Currently, only grant rules are
    supported. A grant rule specifies a role assigned to a set of principals. A list of rules
...
class AccountGroupsClient(AccountGroupsAPI):
    """
    ...
    It is best practice to assign access to workspaces and access-control policies in Unity Catalog
    to groups, instead of to users individually. All Databricks account identities can be assigned
    as members of groups, and members inherit permissions that are assigned to their group.
    """
...
class AccountServicePrincipalsClient(AccountServicePrincipalsAPI):
    """
...
class AccountUsersClient(AccountUsersAPI):
    """
...
class CurrentUserClient(CurrentUserAPI):
    """
...
class GroupsClient(GroupsAPI):
    """
    Groups simplify identity management, making it easier to assign access to Databricks workspace,
    data, and other securable objects.

    It is best practice to assign access to workspaces and access-control policies in Unity Catalog
    to groups, instead of to users individually. All Databricks workspace identities can be assigned
    as members of groups, and members inherit permissions that are assigned to their group.
    """
...
class PermissionMigrationClient(PermissionMigrationAPI):
    """
...
class PermissionsClient(PermissionsAPI):
    """
...
class ServicePrincipalsClient(ServicePrincipalsAPI):
    """
    Identities for use with jobs, automated tools, and systems such as scripts, apps, and CI/CD
    platforms. Databricks recommends creating service principals to run production jobs or modify
    production data. If all processes that act on production data run with service principals,
...
class UsersClient(UsersAPI):
    ...
    to Databricks workspace, admins can terminate the user in your identity provider and that
    user’s account will also be removed from Databricks workspace. This ensures a consistent
...
class WorkspaceAssignmentClient(WorkspaceAssignmentAPI):
    """
