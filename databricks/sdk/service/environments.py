...
class WorkspaceBaseEnvironment:
    ...
    base_environment_cache: Optional[List[WorkspaceBaseEnvironmentCache]] = None
    """List of materialized environments for different architectures."""
    ...
    def from_dict(cls, d: Dict[str, Any]) -> WorkspaceBaseEnvironment:
        ...
        return cls(
            base_environment_cache=_repeated_dict(d, "base_environment_cache", WorkspaceBaseEnvironmentCache),
...
@dataclass
class WorkspaceBaseEnvironmentCache:
    """Materialized environment information for a WorkspaceBaseEnvironment."""

    status: Optional[WorkspaceBaseEnvironmentCacheStatus] = None
    """Status of this materialized environment entry."""

    def as_dict(self) -> dict:
        """Serializes the WorkspaceBaseEnvironmentCache into a dictionary suitable for use as a JSON request body."""
        body = {}
        if self.status is not None:
            body["status"] = self.status.value
        return body
    ...
    def as_shallow_dict(self) -> dict:
        """Serializes the WorkspaceBaseEnvironmentCache into a shallow dictionary of its immediate attributes."""
        body = {}
        if self.status is not None:
            body["status"] = self.status
        return body
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> WorkspaceBaseEnvironmentCache:
        """Deserializes the WorkspaceBaseEnvironmentCache from a dictionary."""
        return cls(status=_enum(d, "status", WorkspaceBaseEnvironmentCacheStatus))
