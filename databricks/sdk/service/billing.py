...
class CreateLogDeliveryConfigurationParams:
    ...
    - ``BILLABLE_USAGE`` — Configure `billable usage log delivery
      <https://docs.databricks.com/administration-guide/account-settings/billable-usage-delivery.html>`__.
      For the CSV schema, see the `View billable usage
      <https://docs.databricks.com/administration-guide/account-settings/usage.html>`__.
    - ``AUDIT_LOGS`` — Configure `audit log delivery
      <https://docs.databricks.com/administration-guide/account-settings/audit-logs.html>`__. For
      the JSON schema, see `Configure audit logging
      <https://docs.databricks.com/administration-guide/account-settings/audit-logs.html>`__"""

    output_format: OutputFormat
    """The file type of log delivery.
    
    - If ``log_type`` is ``BILLABLE_USAGE``, this value must be ``CSV``. Only the CSV
      (comma-separated values) format is supported. For the schema, see the `View billable usage
      <https://docs.databricks.com/administration-guide/account-settings/usage.html>`__
    ...
    Defaults to ``ENABLED``. You can enable or disable the configuration later. Deletion of a
    configuration is not supported, so disable a log delivery configuration that is no longer
    needed."""
...
class LogDeliveryConfigStatus(Enum):
    ...
    ``ENABLED``: All dependencies have executed and succeeded ``DISABLED``: At least one dependency
    has succeeded"""
...
class LogDeliveryStatus:
    ...
    - ``SYSTEM_FAILURE``: The latest attempt of log delivery failed because of an Databricks
      internal error. Contact support if it doesn't go away soon.
...
class BudgetPolicyAPI:
    ...
    def create(self, *, policy: Optional[BudgetPolicy] = None, request_id: Optional[str] = None) -> BudgetPolicy:
        ...
        :param policy: :class:`BudgetPolicy` (optional)
          ...
          be provided, custom_tags may need to be provided depending on the cloud provider. All other fields
          are optional.
    ...
    def update(
        ...
        :param policy: :class:`BudgetPolicy`
          The policy to update. ``creator_user_id`` cannot be specified in the request. All other fields must
...
class LogDeliveryAPI:
    ...
    are *billable usage logs* (AWS only) and *audit logs* (AWS and GCP). This feature is in Public Preview.
    ...
    1. **Create storage**: In AWS, `create a new AWS S3 bucket
       <https://docs.databricks.com/administration-guide/account-api/aws-storage.html>`__ with a specific
       bucket policy. Using Databricks APIs, call the Account API to create a storage configuration object
       that uses the bucket name.
    2. **Create credentials**: In AWS, create the appropriate AWS IAM role. For full details, including the
       required IAM role policies and trust relationship, see `Billable usage log delivery
       <https://docs.databricks.com/administration-guide/account-settings/billable-usage-delivery.html>`__.
       Using Databricks APIs, call the Account API to create a credential configuration object that uses the
       IAM role's ARN.
    3. **Create log delivery configuration**: Using Databricks APIs, call the Account API to create a log
       delivery configuration that uses the credential and storage configuration objects from previous steps.
       You can specify if the logs should include all events of that log type in your account (*Account level*
       delivery) or only events for a specific set of workspaces (*workspace level* delivery). Account level
       log delivery applies to all current and future workspaces plus account level logs, while workspace
       level log delivery solely delivers logs related to the specified workspaces. You can create multiple
       types of delivery configurations per account.

    For billable usage delivery (AWS only):

    - For more information about billable usage logs, see `Billable usage log delivery
      <https://docs.databricks.com/administration-guide/account-settings/billable-usage-delivery.html>`__. For
      the CSV schema, see the `Usage page
      <https://docs.databricks.com/administration-guide/account-settings/usage.html>`__.
    - The delivery location is ``<bucket-name>/<prefix>/billable-usage/csv/``, where ``<prefix>`` is the name
      of the optional delivery path prefix you set up during log delivery configuration. Files are named
      ``workspaceId=<workspace-id>-usageMonth=<month>.csv``.
    - All billable usage logs apply to specific workspaces (*workspace level* logs). You can aggregate usage
      for your entire account by creating an *account level* delivery configuration that delivers logs for all
      current and future workspaces in your account.
    - The files are delivered daily by overwriting the month's CSV file for each workspace.

    For audit log delivery (AWS and GCP):

    - For more information about about audit log delivery, see Audit log delivery `AWS
      <https://docs.databricks.com/administration-guide/account-settings/audit-logs.html>`__ or `GCP
      <https://docs.databricks.com/gcp/en/admin/account-settings/audit-logs>`__, which includes information
      about the used JSON schema.
    - The delivery location is
      ``<bucket-name>/<delivery-path-prefix>/workspaceId=<workspaceId>/date=<yyyy-mm-dd>/auditlogs_<internal-id>.json``.
      Files may get overwritten with the same content multiple times to achieve exactly-once delivery.
    - If the audit log delivery configuration included specific workspace IDs, only *workspace-level* audit
      logs for those workspaces are delivered. If the log delivery configuration applies to the entire account
      (*account level* delivery configuration), the audit log delivery includes workspace-level audit logs for
      all workspaces in the account as well as account-level audit logs. See Audit log delivery `AWS
      <https://docs.databricks.com/administration-guide/account-settings/audit-logs.html>`__ or `GCP
      <https://docs.databricks.com/gcp/en/admin/account-settings/audit-logs>`__ for details.
    ...
    def create(
        ...
        to your storage location. This requires that you already created a credential object (which
        encapsulates a cross-account service IAM role) and a storage configuration object (which encapsulates
        an S3 bucket).

        For full details, including the required IAM role policies and bucket policies, see `Deliver and
        access billable usage logs
        <https://docs.databricks.com/administration-guide/account-settings/billable-usage-delivery.html>`__ or
    ...
    def patch_status(self, log_delivery_configuration_id: str, status: LogDeliveryConfigStatus):
        ...
        :param status: :class:`LogDeliveryConfigStatus`
          Status of log delivery configuration. Set to ``ENABLED`` (enabled) or ``DISABLED`` (disabled).
          Defaults to ``ENABLED``. You can enable or disable the configuration later. Deletion of a
          configuration is not supported, so disable a log delivery configuration that is no longer needed.
