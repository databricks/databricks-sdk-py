...
@dataclass
class CatalogConfig:
    """Data Classification configuration for a Unity Catalog catalog.

    This message follows the "At Most One Resource" pattern: at most one CatalogConfig exists per
    catalog. It combines aspects of both singleton and CRUD resources: - Full CRUD operations are
    supported: Create enables Data Classification, Delete disables it - Like a singleton, it has no
    unique identifier of its own and uses its parent catalog's identifier (catalog_name)"""
    ...
    """Resource name in the format: catalogs/{catalog_name}/config Set by server in responses. Required
    in Update requests to identify the resource."""
...
class CatalogConfigSchemaNames:
    """Wrapper message for a list of schema names. Required because proto2 doesn't allow repeated
    fields directly in oneof."""
...
class DataClassificationAPI:
    ...
    def create_catalog_config(self, parent: str, catalog_config: CatalogConfig) -> CatalogConfig:
        ...
        Creates a new config resource, which enables Data Classification for the specified catalog. The config
    ...
    def delete_catalog_config(self, name: str):
        ...
        Deletes the config resource, which disables Data Classification for the specified catalog.

        This operation is idempotent: deleting a non-existent config succeeds. However, if the catalog itself
        doesn't exist, returns RESOURCE_DOES_NOT_EXIST.
    ...
    def get_catalog_config(self, name: str) -> CatalogConfig:
        ...
        Returns the CatalogConfig resource for the specified catalog. If the config doesn't exist (Data
        Classification not enabled), returns RESOURCE_DOES_NOT_EXIST error.
    ...
    def update_catalog_config(self, name: str, catalog_config: CatalogConfig, update_mask: FieldMask) -> CatalogConfig:
        ...
        Updates fields specified in the update_mask. The config must already exist. Use field mask to perform
        partial updates of the configuration.

        :param name: str
          Resource name in the format: catalogs/{catalog_name}/config Set by server in responses. Required in
          Update requests to identify the resource.
        :param catalog_config: :class:`CatalogConfig`
