...
from .catalog import (AccountMetastoreAssignmentsAPI, AccountMetastoresAPI,
                      AccountStorageCredentialsAPI, ArtifactAllowlistsAPI,
                      CatalogsAPI, ConnectionsAPI, CredentialsAPI,
                      ExternalLocationsAPI, FunctionsAPI, GrantsAPI,
                      MetastoresAPI, ModelVersionsAPI, OnlineTablesAPI,
                      QualityMonitorsAPI, RegisteredModelsAPI,
                      ResourceQuotasAPI, SchemasAPI, StorageCredentialsAPI,
                      SystemSchemasAPI, TableConstraintsAPI, TablesAPI,
                      TemporaryTableCredentialsAPI, VolumesAPI,
                      WorkspaceBindingsAPI)

_LOG = logging.getLogger(__name__)


class AccountMetastoreAssignmentsClient(AccountMetastoreAssignmentsAPI):
    """
...
class AccountMetastoresClient(AccountMetastoresAPI):
    """
...
class AccountStorageCredentialsClient(AccountStorageCredentialsAPI):
    """
...
class ArtifactAllowlistsClient(ArtifactAllowlistsAPI):
    """
...
class CatalogsClient(CatalogsAPI):
    """
...
class ConnectionsClient(ConnectionsAPI):
    """
    Connections allow for creating a connection to an external data source.

    A connection is an abstraction of an external data source that can be connected from Databricks
    Compute. Creating a connection object is the first step to managing external data sources within
    Unity Catalog, with the second step being creating a data object (catalog, schema, or table)
    using the connection. Data objects derived from a connection can be written to or read from
    similar to other Unity Catalog data objects based on cloud storage. Users may create different
    types of connections with each connection having a unique set of configuration options to
    support credential management and other settings.
    """
...
class CredentialsClient(CredentialsAPI):
    """
...
class ExternalLocationsClient(ExternalLocationsAPI):
    """
...
class FunctionsClient(FunctionsAPI):
    """
    Functions implement User-Defined Functions (UDFs) in Unity Catalog.

    The function implementation can be any SQL expression or Query, and it can be invoked wherever a
    table reference is allowed in a query. In Unity Catalog, a function resides at the same level as
    a table, so it can be referenced with the form
...
class GrantsClient(GrantsAPI):
    ...
    Securable objects in Unity Catalog are hierarchical and privileges are inherited downward. This
    means that granting a privilege on the catalog automatically grants the privilege to all current
    and future objects within the catalog. Similarly, privileges granted on a schema are inherited
    by all current and future objects within that schema.
    """
...
class MetastoresClient(MetastoresAPI):
    """
    A metastore is the top-level container of objects in Unity Catalog. It stores data assets
    (tables and views) and the permissions that govern access to them. Databricks account admins can
    create metastores and assign them to Databricks workspaces to control which workloads use each
    metastore. For a workspace to use Unity Catalog, it must have a Unity Catalog metastore
    attached.
    ...
    NOTE: This metastore is distinct from the metastore included in Databricks workspaces created
...
class ModelVersionsClient(ModelVersionsAPI):
    """
...
class OnlineTablesClient(OnlineTablesAPI):
    """
...
class QualityMonitorsClient(QualityMonitorsAPI):
    """
...
class RegisteredModelsClient(RegisteredModelsAPI):
    """
    Databricks provides a hosted version of MLflow Model Registry in Unity Catalog. Models in Unity
    Catalog provide centralized access control, auditing, lineage, and discovery of ML models across
    Databricks workspaces.
    ...
    (MLflow models). Creating new model versions currently requires use of the MLflow Python client.
    Once model versions are created, you can load them for batch inference using MLflow Python
    client APIs, or deploy them for real-time serving using Databricks Model Serving.

    All operations on registered models and model versions require USE_CATALOG permissions on the
    ...
    * To create a registered model, users must additionally have the CREATE_MODEL permission on the
    target schema. * To view registered model or model version metadata, model version data files,
    or invoke a model version, users must additionally have the EXECUTE permission on the registered
    model * To update registered model or model version tags, users must additionally have APPLY TAG
    permissions on the registered model * To update other registered model or model version metadata
    (comments, aliases) create a new model version, or update permissions on the registered model,
    users must be owners of the registered model.
...
class ResourceQuotasClient(ResourceQuotasAPI):
    """
...
class SchemasClient(SchemasAPI):
    """
    A schema (also called a database) is the second layer of Unity Catalog’s three-level
    namespace. A schema organizes tables, views and functions. To access (or list) a table or view
    in a schema, users must have the USE_SCHEMA data permission on the schema and its parent
    catalog, and they must have the SELECT permission on the table or view.
    """
...
class StorageCredentialsClient(StorageCredentialsAPI):
    """
...
class SystemSchemasClient(SystemSchemasAPI):
    """
...
class TableConstraintsClient(TableConstraintsAPI):
    """
...
class TablesClient(TablesAPI):
    ...
    A table can be managed or external. From an API perspective, a __VIEW__ is a particular kind of
    table (rather than a managed or external table).
    """
...
class TemporaryTableCredentialsClient(TemporaryTableCredentialsAPI):
    """
    ...
    reducing the risk ofunauthorized access or misuse. To use the temporary table credentials API, a
    ...
    on the schema for security reason.
    """
...
class VolumesClient(VolumesAPI):
    """
    ...
    science, working with libraries that require access to the local file system on cluster
...
class WorkspaceBindingsClient(WorkspaceBindingsAPI):
    """
